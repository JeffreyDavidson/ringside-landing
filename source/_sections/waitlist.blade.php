<section class="mx-auto grid w-[calc(100%-6rem)] max-w-[80rem] grid-cols-[1.25fr_1fr] items-center gap-24 border-t-[3px] border-t-ringside-red bg-ringside-surface-index py-[clamp(4rem,7vw,7rem)] max-[900px]:grid-cols-1 max-[900px]:gap-10 max-[767px]:w-[calc(100%-2.5rem)]" id="waitlist" aria-labelledby="closing-title">
    ...
    <div>
        ...
        <form class="mt-6 flex flex-wrap gap-3" onsubmit="handleWaitlist(event)">
            <label class="sr-only" for="waitlist-email">Email address</label>
            <input class="min-h-14 min-w-0 flex-1 border border-ringside-line-bright bg-ringside-surface-card px-4 text-ringside-ink placeholder:text-ringside-muted-subtle focus:border-ringside-white focus:outline-3 focus:outline-ringside-white focus:outline-offset-2 max-[520px]:basis-full" id="waitlist-email" name="email" type="email" placeholder="you@example.com" required>
            <button class="inline-flex min-h-14 items-center justify-center gap-4 border border-ringside-red bg-ringside-red px-7 py-3.5 text-base font-bold leading-[1.4] text-ringside-white transition-colors hover:border-ringside-red-dark hover:bg-ringside-red-dark" type="submit">Join the founding class <x-icon.arrow-up-right /></button>
        </form>
        <p class="mt-3 text-sm text-ringside-muted-subtle" id="waitlist-status" aria-live="polite">No spam. Just launch updates.</p>
